...
class FirestoreUserController {
  static Future<void> addUser(String uid, String name) async { // Add new user to firestore
    ...
    return await Geolocator.getCurrentPosition(desiredAccuracy: LocationAccuracy.high).then((Position position) =>
      users
        .add({'uid': uid, 'name': name, 'location': GeoPoint(position.latitude, position.longitude)})
