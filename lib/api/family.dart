...
class FirestoreFamilyController {
  static Future<List<Map<String, dynamic>>> getFamilyUsers() async {
    ...
    DocumentReference family =
        (await FirestoreUserController.getUserDataById(user.uid))['family'];
    for (QueryDocumentSnapshot element
        in (await family.collection('users').get()).docs) {
      Map<String, dynamic> user =
          (await (element.data()['user'] as DocumentReference).get()).data();
      users.add(user);
    }
    return users;
  }
  ...
  static Future<void> addFamily(String name) async {
    ...
    DocumentReference userDocument =
        await FirestoreUserController.getUserDocument(user.uid);
    CollectionReference families =
        FirebaseFirestore.instance.collection('family');
    DocumentReference newFamily = await families
        .add({'code': FamilyCodeGenerator.generateCode(), 'name': name});
  ...
  static Future<void> leaveFamily(String uid) async {
    DocumentReference userDocument =
        await FirestoreUserController.getUserDocument(uid);
    DocumentReference familyDocument =
        (await userDocument.get()).data()['family'];
    String collectionId = (await familyDocument
            .collection('users')
            .where('user', isEqualTo: userDocument)
            .get())
        .docs[0]
        .id;
    ...
    if ((await userDocument.get()).data()['role'] == 'OWNER') {
      List<Map<String, dynamic>> users =
          await FirestoreFamilyController.getFamilyUsers();
      if (users.length > 0) {
        DocumentReference newOwnerDocument =
            await FirestoreUserController.getUserDocument(users[0]['uid']);
